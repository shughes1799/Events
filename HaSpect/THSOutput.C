...
void  THSOutput::HSSlaveBegin(TList* input,TList* output){
 ...
 if(!fSelOutput){
  ...
  if(!fCodeList){ //running on PROOF
    ...
    Error(" THSOutput::HSSlaveBegin"," no code list found cannot save source to file");
  }
...
void THSOutput::HSTerminate(){
  ...
  if(fOutName.EndsWith(".root")){//Just one file
    ...
    TString InDirName=gSystem->DirName(((TEntryList*)elist->GetLists()->At(0))->GetFileName()); 
	...
	TEntryList* eltemp=elist->GetEntryList(elist->GetTreeName(),InDirName+"/"+elpofile->GetName());
...
void THSOutput::FinishOutput(){
  ...
  if (fFile) {
    ...
    fFile->cd();
    if (!fOutTree) {
      // Error("THSOutput::FinishOutput", "'outtree is undefined!");
   ...
   //Loop over outout and look for histograms to save
    TIter next(fSelOutput);
    TObject* ho=0;
    TH1* hf=0;
    while((ho=next())){
